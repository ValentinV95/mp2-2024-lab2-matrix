...
public:
    TDynamicMatrix(size_t s = 1) : TDynamicVector<TDynamicVector<T>>(s)
    {
        if(sz <= 0 || sz > MAX_MATRIX_SIZE){
            throw out_of_range("size should be greater than zero and less max matrix size");
        }
        for (size_t i = 0; i < sz; i++)
    ...
    {
        if (sz != v.size()) {
            throw logic_error("sizes matrix and vector should be equal");
        }
        TDynamicVector<T> tmp(sz);
        for (size_t i = 0; i < sz; i++) {
            tmp[i] = pMem[i] * v;
    ...
    {
        if (sz != m.sz) {
            throw logic_error("sizes matrices should be equal");
        }
        TDynamicMatrix<T> tmp(sz);
        for (size_t i = 0; i < sz; i++) {
